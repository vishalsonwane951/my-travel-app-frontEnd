...
/* ------------------------------------------------------------------
   ...
   Why: this used to call RapidAPI directly from the client, with the API
   key embedded in the frontend bundle (and even printed to the console).
   Now every call goes to Backend/Routes/HotelSearchRoutes.js, which holds
   the RapidAPI key server-side and, for autocomplete/search/property-details,
   checks MongoDB for a matching prior search (30-day TTL) before ever
   calling Xeni — see Backend/Controllers/hotelSearchController.js.
