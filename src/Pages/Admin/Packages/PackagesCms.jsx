...
const PackagesCms = () => {
  ...
  const saveItinerary = async () => {
    await adminApi.put(`/packages/${itineraryEditor._id}/itinerary`, {
      itinerary,
    });
  ...
  const uploadGalleryImages = async () => {
    ...
    try {
      ...
      const { data } = await adminApi.post(
        `/packages/${galleryEditor._id}/gallery`,
        fd,
      );
  ...
  const submitCsv = async () => {
    try {
      const { data } = await adminApi.post("/packages/bulk-import", {
        csv: csvText,
      });
      alert(data.message);
      setCsvOpen(false);
  ...
  return (
    <div>
            ...
            {!loading &&
              packages.map((p) => (
                <tr key={p._id}>
                  ...
                  <td>
                    <button
                      className={`admin-badge ${p.active ? "badge-confirmed" : "badge-rejected"}`}
                  ...
                  <td>
                    <button
                      className={`admin-badge ${p.featured ? "badge-confirmed" : "badge-pending"}`}
                      style={{ border: "none", cursor: "pointer" }}
        ...
        >
          <div className="admin-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSave}>
              ...
              <div className="admin-form-row">
                <label>Strike Price (₹, optional)</label>
                <input
                  className="admin-input"
                  type="number"
              ...
              <div className="admin-form-row">
                <label>
                  Durations (semicolon separated, e.g. "4N/5D; 6N/7D")
                </label>
                <input
                  className="admin-input"
                  value={form.durations}
                  onChange={(e) =>
                    setForm({ ...form, durations: e.target.value })
                  }
                />
              </div>
              <div className="admin-form-row">
              ...
              <div className="admin-form-row">
                <label>
                  Included (semicolon separated, e.g. "Breakfast; Airport
              ...
              <div className="admin-form-row">
                <label>
                  Not Included (semicolon separated, e.g. "Flights; Personal
                  Expenses")
                </label>
                <textarea
                  className="admin-input"
      ...
      {itineraryEditor && (
        <div
          className="admin-modal-backdrop"
          onClick={() => setItineraryEditor(null)}
        >
          <div
          ...
          >
                ...
                <div className="admin-form-row">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      copy[i].day = Number(e.target.value);
                      setItinerary(copy);
                    }}
                  />
                </div>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
              <button className="admin-btn" onClick={saveItinerary}>
                Save Itinerary
              </button>
              <button
                className="admin-btn secondary"
        ...
        >
          ...
          >
            ...
            >
              ...
              {(!galleryEditor.gallery ||
                galleryEditor.gallery.length === 0) && (
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
              <button
                ...
                disabled={!galleryFiles.length || galleryUploading}
              >
                {galleryUploading
                  ? "Uploading…"
        ...
        <div className="admin-modal-backdrop" onClick={() => setCsvOpen(false)}>
          ...
          >
            ...
            <p style={{ fontSize: 12.5, color: "#888" }}>
              Columns: title, location, type, destination, price, strikePrice,
              durations (semicolon-separated), description, highlights
              (semicolon-separated). First row must be the header.
            </p>
            <textarea
              className="admin-input"
              rows={10}
              ...
              placeholder={
                "title,location,type,destination,price\nGoa Beach Getaway,Goa,domestic,Goa,12999"
              }
            />
            <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
              <button className="admin-btn" onClick={submitCsv}>
                Import
              </button>
              <button
                className="admin-btn secondary"
